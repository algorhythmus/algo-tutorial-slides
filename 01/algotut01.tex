...
\begin{frame}[t]{Algorithmenanalyse}
	...
	\TrueQuestion{$\forall c_1, c_2 \in \N, \ f: \N \functionto \N_+: \quad c_1 \cdot f(n) + c_2 \in O(f(n)).$} % f muss nach N_+ gehen, da es mit 0 als Funktionswert kaputt geht!
	
	\FalseQuestionE{$\forall c \in \N, \ f: \N \functionto \N_+: \quad (f(n))^c \in \omega(f(n)).$}{(\crash für $c = 1$.)}
\end{frame}
...
\begin{frame}{SelectionSort -- Beweis Invariante}
	\begin{overlayarea}{\textwidth}{.60\textwidth}
		...
		\pause
		\only<3-|handout:2>{\hanging\textbf{IV}. \textit{($i > 1$)}: \ Die Invariante gilt zu Beginn des Durchlaufs $i$. \\}
		\pause
