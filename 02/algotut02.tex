...
\begin{frame}{Quiz}
	\thasse{\Socrative}
	\FalseQuestion{Für alle Funktionen $f, g$ gilt $f < g$ oder $f = g$ oder $f > g$}
...
\begin{frame}{Aufgaben Master-Theorem}
	...
	\begin{itemize}
		...
		\item Also: 
		\begin{align*}
			\log_{4}a &< \log_{2}7 \gdw 4^{\log_{4}a} < 4^{\log_{2}7} = 7^{\log_{2} 4} = 7^2 = 49 \\ \pause  %[3] for no pause Pause is NEEDED for rendering!
			\Impl a &= 48.
		\end{align*}
